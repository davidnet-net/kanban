<script lang="ts">
	...
	async function setupWS(cardId: number) {
		const wsProtocol = kanbanapiurl.startsWith("https") ? "wss" : "ws";
		const wsUrl = `${wsProtocol}://${kanbanapiurl.replace(/^https?:\/\//, "")}card/live/${cardId}`;

		socket = new WebSocket(wsUrl);

		socket.onopen = () => console.log("WebSocket connected to card", cardId);
		...
		socket.onmessage = async (e) => {
			try {
				...
				switch (payload.type) {
					case "update_card":
						console.log("Card update received via WS, reloading card data...");
						try {
							// 1. Update the main card details
		...
		socket.onclose = () => {
			...
			setTimeout(() => setupWS(cardId), 2000);
		};
	}
					...
					{:else}
							...
							<div class="activity">
								...
								<div>
									...
									<div class="activity-meta">
										{$_("kanban.components.cardoverlay.text.created_card_on", { values: { date: creation_date } })}
									</div>
								</div>
							</div>
						...
						<div class="comment-input-area">
							...
							<div style="margin-top: 0.5rem; display: flex; justify-content: flex-end;">
								<Button appearance="primary" disabled={!new_comment.trim() || isSubmittingComment} onClick={createComment}>
...
{#if showBlanket}
	<!-- svelte-ignore a11y_click_events_have_key_events -->
	<div class="blanket" onclick={(e) => blanketClosable && e.target === e.currentTarget && closeOverlay()} tabindex="-1" aria-modal="true">
		{@render cardContent()}
	</div>
{:else}
	{@render cardContent()}
